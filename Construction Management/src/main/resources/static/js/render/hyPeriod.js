...
period.buildHeader = function(){
	...
	period.fileupload = new Ext.ux.form.FileUploadField({
	    id: 'kbURL',
	    allowBlank: false,
	    name: 'documentURL',
	    hideLabel: true,
	    ...
	    listeners: {
	        fileselected: function (fp, fileName) {
	        	if(fp.fileInput.dom.files[0].size <= 10000){
	        		var reader = new FileReader();
		            reader.onload = function (e) {
		                period.panel.update('<img src='+e.target.result+' style="height:150px; width:150px"; />');
		            }
		            reader.readAsDataURL(fp.fileInput.dom.files[0]);
	        	}else{
	        		Ext.Msg.alert(__messages['title.information'], 'Image over size.', function(){
	        			period.fileupload.setValue('');
	        		});
	        	}
	        }
	    }
...
//		  border : false,
	...
	period.periodContainer = new Ext.Container({
        ...
        items: [{
                ...
                layout: 'form',
                columnWidth: 0.4,
                items: [period.periodName,period.fileupload, period.panel]
            },{
                xtype: 'container',
                layout: 'form',
                columnWidth: 0.4,
...
period.buildPeriodDetail = function(){
	...
	period.fbutton = new Ext.ux.form.FileUploadField({
        buttonOnly: true
    });
...
//            	}
            	...
            	if(BeanUtils.equals(e.field, 'hyPeriodDate')){
            		...
            		e.record.data.hyPeriodDate = e.value;
            		period.periodDateGrid.setValue(new Date());
            	}
