...
Ext.ss.grid.CheckboxSelectionModel = Ext.extend(Ext.grid.AbstractSelectionModel, {
	...
	dirtyOnChecked: false,
	disabledDataIndex : 'disabledFlag',
	lockOnInit : false,
	checkOnly : false,
	singleSelect : false,
	onEnterHorizontal : false,
	autoAddRecord:false,
	disabledValue:true,
	defaultAddRecord:{},
    ...
    renderer : function(v, p, record, rowIndex, colIndex, store) {
    	return this.renderRowCheckbox(rowIndex,false,(record.get(this.disabledDataIndex) === this.disabledValue) ,p);
    },
    initEvents : function(){
    ...
    onRefresh : function(){
        ...
        if(this.haveFieldDataIndex === true && this.dirtyOnChecked === true)
        {
    ...
    getHdElement : function() {
    	var dom = Ext.getDom('x-grid3-hd-checker-'+this.__id);
    	return Ext.fly(dom.parentNode);
    },
        ...
        if(r && this.fireEvent('beforerowselect', this, index, keepExisting, r) !== false){
            ...
            if(!checkbox.hasClass('x-grid3-row-ss-checker-disabled')){
            	...
            	if(this.haveFieldDataIndex) {
            		if(this.dirtyOnChecked){
            			r.set(this.dataIndex,true);
            		}else{
    ...
    onEditorKey : function(field, e){
        ...
        function processOnEditorKey(newCell, r, c, last, g) {
        	if(newCell){
        		...
        		if(t instanceof Ext.PagingToolbar){
	        		var activePage = Math.ceil((t.cursor + t.pageSize) / t.pageSize);
	        		var totalCount = g.store.getTotalCount();
	        		var totalPage = Math.ceil(totalCount / t.pageSize)
	        		if(shift){
	        			if(t.cursor - t.pageSize >= 0){
		        			t.doLoad(t.cursor - t.pageSize,{
	        					callback:function(){
	        						g.processActiveLastCell(t.pageSize - 1)
	        					},scope:this
	        				});
	        			}else {
	        				g.processActiveFirstCell(0);
	        			}
	        		}else{
//	        			if(totalPage === activePage && last.row+1 <= t.pageSize){
//	        				if(this.autoAddRecord === true){
//	        					g.add(Ext.apply({},this.defaultAddRecord));
//	        				}
//	        			} else {
//	        				t.doLoad(t.cursor + t.pageSize,{
//	        					callback:function(){
...
//	        			}
        ...
        if(k == e.TAB){
            ...
            if(this.moveEditorOnEnter !== false){
            	if(this.onEnterHorizontal !== false){
            		 if(shift){
